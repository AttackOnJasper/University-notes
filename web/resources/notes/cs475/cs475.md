...
- Thm (Sherman-Morrisen-Woodbury): $$ A\in\Re^{n*n}, B\in\Re^{m*m}, u\in\Re^{n*m}, V\in\Re^{n*m}, (A + uBV)^{-1} = A^{-1} - A^{-1}u(B^{-1}+v^TA^{-1}u)^{-1}vA^{-1} $$
...
- Psedo code for solving lower triangular matrix O(n^2)
	- Problem: division of $$ l_{i,i} $$ could be 0. Solution: add a small number to all diagonals to avoid such issues / pivoting
...
- Gaussian Elimination psedo code
	...
	- still have the problem of division
		...
		- Answer: compute the **determinent**: $$ det(A) = \sum_{\sigma}(-1)^{sgn(\sigma)} * \prod^{n}_{i=1} A_{i, \sigma(i)} $$

### Thm (LU factorization): let \widetilde{L} and u be the strict lower triangular and upper triangular of the output of GE, then A = L*u, L = \widetilde{L} + I
...
- $$ \begin{aligned}
		...
		u \vec x &= \vec y
\end{aligned} $$
- use forward solve to get y and backward solve to get x, time complexity $$ O(n^3) $$
